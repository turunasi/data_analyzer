...
// #define POSIT_IN "C:/Users/small/Desktop/data/DAT/210701"    //読み込むファイルのパス
// #define POSIT_OUT "C:/Users/small/Desktop/data/DAT2/210701"        //出力先
#define POSIT_IN "../data/210701"    //読み込むファイルのパス
#define POSIT_OUT "../data_2/210701"        //出力先
#define T_STANDBY 0.0        //立ち上がりまでの秒数(整数がいい)
#define FS 96000
...
int main(void) {
    ...
    for (fn = START; fn <= END; fn++) {
        ...
        sprintf(posit_name_in, "%s/%08d.DAT", posit_in, fn);                //読み込むファイルのバイナリデータが入っているパスを作る.
        if ((fp_in = fopen(posit_name_in, "rb")) == NULL) {                 //そのファイルを開く.
            printf("「%s/%08d.DAT」は、ないですね(´・ω・｀)\n", posit_in, fn);
            continue;                                                       //読み込むファイルの番号が連続でなくても大丈夫なように
        }
        else {                                                              //している.
            printf("「%03di.DAT」処理中\n", fn);
        }
        ...
        for (count = 0; count < 1; count++) {
            ...
            //--------------------------------------------------------------------------------
            //分割データ書き込み
            sprintf(posit_name_out, "%s/%08di_2.DAT", posit_out,  fn);        //出力ファイル
            sprintf(posit_name_out2, "%s/%08di_2.LOG", posit_out,  fn);
            printf("%s\n", posit_name_out);
            if ((fp_out = fopen(posit_name_out, "wb")) == NULL) {        //そのファイルを開く.
                printf("DATないですね(´・ω・｀)\n");
                continue;
            }
            if ((fp_out2 = fopen(posit_name_out2, "w")) == NULL) {        //そのファイルを開く.
                printf("LOGないですね(´・ω・｀)\n");
                continue;
            }
            printf("%03d_%03d\n", fn, count + 1);
            printf("\t%03d_%d.BINデータ書き込み\n", fn, count + 1);
