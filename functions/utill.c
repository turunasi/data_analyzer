...
#include "config/variables.h" // グローバル定数を纏めた自作ヘッダ
...
int sort(int fn, int is_initial, char* input_directory_path, char* output_directory_path, int ch_num1, int ch_num2) {
    ...
    for (i = 0; i < c; i++) {
        ...
        if (is_initial == 1) {
            sprintf(output_file_path, "%s/%08di_new.DAT", output_number_path, i + 1);
        } else {
            sprintf(output_file_path, "%s/%08d_new.DAT", output_number_path, i + 1);
        }
